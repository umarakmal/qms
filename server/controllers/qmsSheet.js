...
const storage1 = multer.diskStorage({
  destination: (req, file, callback) => {
    callback(null, "../client/build/uploads/files");
  },
...
exports.findDistinctCalltypes = async (req, res) => {
  try {
    ...
    if (collectionname === y) {
      const z = "$" + abc
      var data = await createModelForNameMapping(collectionname).aggregate([{
        $addFields: {
          inMilliseconds: {
            $subtract: [
              (new Date()),
...
exports.getDataFromResponceForGraph = async (req, res) => {
  try {
    const data = await Responce.aggregate([
      {
        $project: {
          id: 1,
          month: {

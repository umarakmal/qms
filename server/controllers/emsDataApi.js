      ...
      const data = await whole_details_peremp.aggregate([
        {
          $match: {
            cm_id: {
              $nin: ["47", "36", "34", "5", "6", "59", "37", "35", "186", "29"],
            },
          },
        },
        {
    ...
    var res1 = await whole_details_peremp.aggregate([
      ...
      },
      {
        $project: {
          _id: 0,
          EmpDetails: {
            $concat: ["$EmployeeName", "-", "$EmployeeID"],
          },
        },
      },
    ]);
    ...
    const data = await whole_details_peremp.aggregate([
      ...
      },
      {
        $addFields: {
          ageInYears: {
            $floor: {
              $divide: ["$ageInMilliseconds", 31557600000],
            },
          },
        },
      },
        ...
        $addFields: {
          ageInMonths: {
            $floor: {
              $mod: [
                {
                  $divide: ["$ageInMilliseconds", 2628000000],
                },
                12,
              ],
            },
        ...
        $addFields: {
          ageInDays: {
            $floor: {
              $mod: [
                {
                  $divide: ["$ageInMilliseconds", 86400000],
                },
                30,
              ],
            },
...
exports.getProcessForReport = async (req, res) => {
  try {
    ...
    if (emp_type === "Demo") {
      ...
      if (
        EmployeeID === "CE10091236" ||
        EmployeeID === "CE03070003" ||
        flag === 1
      ) {
        var data = await whole_details_peremp.aggregate([
          {
            $group: {
              _id: {
                clientname: "$clientname",
        ...
        EmployeeID === "OFF8408" ||
        EmployeeID === "OFF8409" ||
        EmployeeID === "9266882209" ||
        EmployeeID === "9627329523" ||
        EmployeeID === "8800838362" ||
        EmployeeID === "9625762391" ||
        EmployeeID === "8178433132" ||
        EmployeeID === "9891988449" ||
        EmployeeID === "9205541837" ||
        EmployeeID === "9958602820" ||
        EmployeeID === "7073695246"
      ) {
        const data = await clientMaster.aggregate([
          {
            $match: {
              client_id: "74",
            },
    ...
    var res1 = await employee_map.aggregate([
      ...
      },
      {
        $addFields: {
          inDays: {
            $floor: {
              $divide: ["$inMilliseconds", 86400000],
            },
          },
        },
      },
...
exports.getQaTlTrDecByCmid = async (req, res) => {
  try {
    const cm_id = req.body.cm_id;
    const data = await activeEmpId.aggregate([
      {
        $match: {
          cm_id: cm_id,
          emp_level: "EXECUTIVE",

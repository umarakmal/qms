...
const D_CreateSheet = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    any = formValues;
    const collectionnames = "input5_"+dataSource.collectionname
    if (!process || !collectionnames || !any[0].fieldname || !param[0].parameter || !param[0].subparameter || !param[0].alias ||!param[0].maxmarks || !param[0].opt1 || !param[0].opt2) {
      return false;
    } else {
      const res = await fetch(`/api/create-sheet-dynamic`, {
        method: "POST",
        headers: {
  ...
  const keyUp = (e) => {
    e.preventDefault();
    const collectionnam = e.target.value;
    var collectionnames = "input5_"+collectionnam
    const postDataSource = async () => {
      const res = await fetch("/api/get-dynamic-collectionname-for-dynamic", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
                              ...
                              <div className="row">
                                ...
                                <div className=" form-group col-sm-3">
                                  ...
                                  >
                                    ...
                                    <option  id="yesnumeric" value="Yes">Yes</option>
                                  </select>
                                </div>
                                <div className="form-group col-sm-3">
                                  <label

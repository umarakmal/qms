...
const Report = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!startDate || !endDate) {
      ...
      var flag = 0
      if (isAuth().EmployeeID === isAuth().qh || isAuth().EmployeeID === isAuth().account_head) {
        flag = 1
      }

      const postdata2 = async () => {
        const sheet_name = sheetName
        const res = await fetch(`${url}/api/get-report-data`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            date1, date2, cm_id, sheet_name, auditorid, flag
          }),
        });
        const data = await res.json();
        if (res.status === 422 || !data) {
